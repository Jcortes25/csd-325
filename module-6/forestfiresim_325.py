...
#Adding Lake to the center of the grid
lake_size = 5  
lake_start_row = GRID_SIZE // 2 - lake_size // 2
lake_start_col = GRID_SIZE // 2 - lake_size // 2
for i in range(lake_size):
    for j in range(lake_size):
        grid[lake_start_row + i][lake_start_col + j] = '~'
#Printing Grid
def print_grid():
    os.system('clear')  
    for row in grid:
